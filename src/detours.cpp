...
// ============================================================================
// Legacy acceleration/friction (pre-update) with surf settings + sv_maxvelocity
// ============================================================================
struct LegacyAccelParams
{
	float sv_accelerate   = 10.0f;    // surf
	float sv_airaccelerate= 150.0f;   // surf
	float sv_friction     = 5.2f;     // surf (use 4.0f for more glide)
	float sv_stopspeed    = 80.0f;    // surf
	float sv_maxvelocity  = 10000.0f; // overall clamp; <=0 disables
	bool  enable          = true;
};

static LegacyAccelParams g_LegacyAccelParams {};

static inline void Legacy_ApplyFriction(const LegacyAccelParams& p, float dt, Vector& velXY)
{
	float speed = velXY.Length2D();
...
{
	// No classic 30 wishspeed cap for surf; clamp with sv_maxvelocity later
	float currentSpeed = DotProduct(velXY, wishdir);
	float addSpeed = wishspeed - currentSpeed;
	if (addSpeed <= 0.0f) return;
...
// Compute legacy XY accel/friction using mv->m_outWishVel; preserve Z in caller
...
void FASTCALL Detour_ProcessMovement(CCSPlayer_MovementServices *pThis, void *pMove)
{
	...
	// Legacy XY candidate before engine runs
	Vector preVel;
	...
	if (player->currentMoveData && g_LegacyAccelParams.enable)
	{
		...
		legacyVelCandidate.z = preVel.z; // preserve Z/gravity path
	}

	// Run engine (subtick/gravity/collision/etc.)
	ProcessMovement(pThis, pMove);

	// Override XY with legacy and clamp to sv_maxvelocity
	if (player->currentMoveData && g_LegacyAccelParams.enable)
	{
		Vector postVel;
...
bool IsValidMovementTrace(trace_t &tr, bbox_t bounds, CTraceFilterPlayerMovementCS *filter)
{
	...
	// Post-update quirk: one-way hits appeared; keep this lenient
	if (stuck.m_bStartInSolid /*|| stuck.m_flFraction < 1.0f - FLT_EPSILON*/)
	{
		return false;
	}
...
{
	// Compute legacy TPM and still call engine for side effects, then FORCE legacy result.
	TryPlayerMovePre(ms, pFirstDest, pFirstTrace, bIsSurfing);
	TryPlayerMove(ms, mv, pFirstDest, pFirstTrace, bIsSurfing);

	ZEPlayer *player = g_playerManager->GetPlayer(ms->GetPawn()->m_hController()->GetPlayerSlot());
	if (player && player->tpmOrigin != vec3_invalid && player->tpmVelocity != vec3_invalid)
	{
		player->SetOrigin(player->tpmOrigin);
		player->SetVelocity(player->tpmVelocity);
	}
	// We skip TryPlayerMovePost's conditional override by enforcing legacy every time.
}
